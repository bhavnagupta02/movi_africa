<?php include 'header.php';
	...
	if(isset($_POST) && $_POST['title']){
	    ...
	    $id = $_GET['id'];
        $stmt= $user_home->runQuery("SELECT * FROM jobs WHERE url=:id"); 
	   ...
	   /* echo "<br>";
        ...
        if($_GET['id'] == ''){  
			...
			if($_GET['id'] != ''){
				...
				$stmt = $user_home->runQuery("UPDATE jobs SET 
				    title=:title,
				...
				if($stmt->execute()){
					...
					$Pt = SITE_URL.'manage-job?id='.$url.'&smsg=update';
				...
				$stmt = $user_home->runQuery("INSERT INTO jobs(title,description,end_date,created_at,user_id,url,location,status) VALUES (:title,:description,:end_date,:created_at,:user_id,:url,:location,:status)");
				...
				if($stmt->execute()){
					...
					$Pt = SITE_URL.'manage-job?id='.$url.'&smsg=added';
					echo '<script>window.location.href = "'.$Pt.'"; </script>';
				}
		...
		if($_GET['id'] != ''){
		  ...
		  $userRow = $stmt->fetch(PDO::FETCH_ASSOC);
			  ...
			  if($userRow['user_id'] != $_SESSION['IdSer']){
				  echo '<script>window.location.href="'.SITE_URL.'manage-job";</script>';
			  }
        ...
        <div class="row">
			<div class="col-md-12">	
				...
				<div class="form-group">
                    ...
                    <input type="text" class="field-element" name="title" value="<?php echo $_POST['title'];?>" id="Title" placeholder="Enter Title"/ required>
                ...
                <div class="form-group">
                    <label for="company_name">End Date</label>
                    <input type="date" class="field-element" name="end_date" value="<?php echo $_POST['end_date'];?>" id="Title"  required>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datetimepicker/4.17.37/js/bootstrap-datetimepicker.min.js"></script>
	...
	$("select").on('change',function(e){
	    ...
	    $(document).ready(function() {
          $(function() {
            var dateNow = "<?php echo $userRow['end_date'];?>";
