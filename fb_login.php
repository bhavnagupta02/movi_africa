...
$user_home = new USER();
    ...
    $stmt = $user_home->runQuery("SELECT * FROM users WHERE fb_id=:fb_id");
	$stmt->execute(array(":fb_id"=>$_POST['fb_id']));
	$userRow = $stmt->fetch(PDO::FETCH_ASSOC);
			...
			if($stmt->rowCount() == 1){
			    ...
			    $stmt = $user_home->runQuery("
						INSERT INTO users(
							user_name,first_name,last_name,fb_id,created_at,last_seen,verify,admin_control,reg_type
						) 
						VALUES(
							:user_name,:first_name,:last_name,:fb_id,:created_at,:last_seen,:verify,:admin_control,:reg_type
						)
                ...
                $stmt->bindparam(":fb_id",$_POST['fb_id']);
