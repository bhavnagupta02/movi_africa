<?php include 'header.php';
	...
	if($_GET['to'] != ''){
		...
		foreach($all_post as $ap){
			...
			if($_SESSION['IdSer'] == $ap['f_unique']){
				array_push($pa,array("c"=>$ap['t_unique'],"t"=>$ap['msg_tm'],"m"=>$ap['msg']));
				$To = $ap['t_unique'];
				$Dta.="'".$ap['t_unique']."',";
			}
			else if($_SESSION['IdSer'] == $ap['t_unique']){
				array_push($pa,array("c"=>$ap['f_unique'],"t"=>$ap['msg_tm'],"m"=>$ap['msg']));
				$To = $ap['f_unique'];
				$Dta.="'".$ap['f_unique']."',";
			}
	...
	if($Lf){
		...
		foreach($allst as $apnw){
			array_push($Raa,array('m'=>$apnw['message'],'t'=>$apnw['timer'],'s'=>$apnw['f_un']));
		}	
...
#DashBx #DashBx-Lt-T{position: relative;background: #eee;}
#DashBx #DashBx-Lt-T-mg{height: 60px;background: #aaa;background-size: cover;}
...
#DashBxR{float: left;width: 280px;margin:45px 0 0 10px;}

div#FooBx{display:none;}

#DashBx #DashBx-Lt-T h2{margin: 0;
...
#DashBx #DashBx-Lt ul{margin: 0;padding: 0;list-style: none;border-top: 1px solid #ddd;background: #f2f2f2;margin-top: 22px;}
#DashBx #DashBx-Lt ul li{line-height: 21px;padding: 5px;border-bottom: 1px solid #ddd;}
#DashBx #DashBx-Lt ul li a{}

#DashBx #DashBx-Lt-B{padding: 10px;overflow: hidden;text-align: center;}
#DashBx #DashBx-Lt-B a{font-size: 11px;font-weight: 600;cursor: pointer;}
	#PstDa{}
	.MsgBxD{
border-bottom: 1px solid #d3d3d3;
position: relative;
padding-left: 60px;height:60px;}
	.MsgBxD .MsgBxD-T{padding:0;position: relative;padding-top: 2px;}
	.MsgBxD .MsgBxD-T b{font-size: 13px;padding-left: 1px;}
	.MsgBxD .MsgBxD-T span{position: absolute;
font-size: 11px;
font-weight: 600;
color: #666;
bottom: -33px;
...
border: 1px solid #ddd;
	...
	.MsgBxD .MsgBxD-B{padding: 2px;
font-size: 12px;
font-weight: 600;
line-height: 13px;height: 17px;
overflow: hidden;}

#PstDa h2{font-size: 11px;
text-align: center;
margin: 0;
padding: 5px;
font-weight: 600;}
	...
	.MsgCt .MsgCt-Tp{padding: 2px 3px;
position: relative;
line-height: 21px;}
	.MsgCt .MsgCt-Tp b{}
	.MsgCt .MsgCt-Tp span{
...
display: block;line-height:12px;}
	.MsgCt .MsgCt-Bt{border-top:0;padding:0 5px;}
...
min-height: 60px;}
	...
	#MsgtSd div{overflow:hidden;margin-top:5px;}
	
	
	#MsgtSd button{float: right;
border: 1px solid #aaa;
border-radius: 2px;
cursor: pointer;}
...
padding: 10px 5px;
margin: 0 0 12px;}
		#MyProfile div.img{background-size: cover;
...
position: relative;
height: 100px;}
		#MyProfile span{display: inline-block;
line-height: 48px;
font-size: 16px;}
		#MyProfile .AbTns{text-align:center;}
		#MyProfile .AbTns a{display: inline-block;
...
#LnkHdr{text-align: center;}
#LnkHdr a{display: inline-block;
color: #333;
margin: 1px 5px;transition: unset;
font-size: 13px;cursor:pointer;}


#LnkHdr a:hover{text-decoration:none;color:#2ab4a1;}	
...
@media screen and (min-width:1120px) and (max-width:1216px){
	#DashBx{width:1112px;}
	#DashBxR{width: 200px;}
	#MyProfile .AbTns a{width:80px;}
}

@media screen and (min-width:912px) and (max-width:1119px){
	#DashBx{width:900px;}
	#DashBxR{display:none;}
}

@media screen and (min-width:320px) and (max-width:911px){
	#DashBx{width:100%;}
	#DashBxL{width:100%;}
	#Msg-DashBx-Ctr {width: 240px;}
	#DashBx #Msg-DashBx-Rt{width: calc(100% - 240px);left: 240px;}
	#Msg-DashBx-Rt #Msg-Ct{width:100%;}
	#MsgtSd{width:100%;}
	#DashBxR{display:none;}
}

@media screen and (min-width:608px) and (max-width:718px){
	#Msg-DashBx-Ctr{width:160px;}
	.MsgBxD .MsgBxD-T a img {
		width: 28px;
		left: -30px;top: 10px;height:28px;
	}
	...
	.MsgBxD{padding-left: 34px;height: 48px;}
	.MsgBxD .MsgBxD-T b a{font-size: 11px;}
	
	.MsgBxD .MsgBxD-T span{bottom: -12px;left: 2px;}
	.MsgBxD .MsgBxD-B{position: absolute;
...
#ShowMsgMb{display:none;position: fixed;
left: 0px;
z-index: 400;
background: #32517c;
color: #fff;
...
#ShowMsgMb i.fa-envelope{}
#ShowMsgMb i.fa-th-list{}
</style>
...
<div id="SearchBx"><input onkeyup="FtchLc()" placeholder="Search" type="text"/></div>
	<div id="PstDa">
		
	</div>
</div>
</div>
<div id="Msg-DashBx-Rt" class="col-lg-9">
    <div id="Msg-Ct"></div>
<?php	
	if($_GET['to'] != ''){
	...
	var saveData = $.ajax({
	  ...
	  success: function(resultData){
			...
			for(var i in Sj){
				...
				$("#Msg-Ct").append('<div class="MsgCt '+msg_right+'" data-id="msg-'+Sj[i]['t']+'">'+Im+'<div class="MsgCt-Tp"><b>'+(Sj[i]['s'] == Usr ? Ur : JsA[Sj[i]['s']]['n'])+'</b> <span data-livestamp="'+Sj[i]['t']+'"></span></div><div class="MsgCt-Bt">'+Sj[i]['m']+'</div></div>');
			}
...
function UpdateMsg(r){
	...
	for(var k in t){
		if($("#MsgBxD-"+t[k]).attr('data-value') != j[t[k]]['t']){
			...
			$("#MsgBxD-"+t[k]+' .MsgBxD-B').html(j[t[k]]['m']);
			$("#MsgBxD-"+t[k]+' span').attr('data-livestamp',j[t[k]]['t']);
	...
	<script>
		...
		for(var i=0;i<Ay.length;i++){
			...
			MsgCt='<div class="MsgCt '+msg_right+'" data-id="msg-'+Ay[i]['t']+'">'+Im+'<div class="MsgCt-Tp"><b>'+(Ay[i]['s'] == Usr ? Ur : JsA[Ay[i]['s']]['n'])+'</b> <span <span data-livestamp="'+Ay[i]['t']+'"></span></div><div class="MsgCt-Bt">'+Ay[i]['m']+'</div></div>';
		...
		$("#MsgtSd button").on('click',function(){
			...
			var saveData = $.ajax({
				  ...
				  success: function(resultData){
						...
						Tmer=Js['t'];
						$("#MsgBxD-"+To+' .MsgBxD-T span').attr('data-livestamp',Js['t']);
						$("#MsgBxD-"+To+' .MsgBxD-B').html($("#MsgtSd textarea").val());
						$("#MsgtSd textarea").val('');
						var objDiv = document.getElementById("Msg-Ct");
						objDiv.scrollTop = objDiv.scrollHeight;
				  }
		...
		for(var i=0;i<Js.length;i++){
			...
			if(Js[i]['m'] != ''){
				Msg=Js[i]['m'];
				PstDa+='<div class="MsgBxD" id="MsgBxD-'+Js[i]['c']+'" data-key="'+Js[i]['c']+'" data-value="'+Js[i]['t']+'"><div class="MsgBxD-T"><b><a href="'+Page+'message.php?to='+Js[i]['c']+'"><img src="'+mug+'"/>'+JsA[Js[i]['c']]['n']+'</b></a> '+ ( (Js[i]['t'] != 0) ? '<span data-livestamp="'+Js[i]['t']+'"></span>': '' )+'</div><div class="MsgBxD-B">'+Msg+'</div></div>';
			}
				else{
				PstDa+='<div style="display:none;" class="MsgBxD" id="MsgBxD-'+Js[i]['c']+'" data-key="'+Js[i]['c']+'" data-value="'+Js[i]['t']+'"><div class="MsgBxD-T"><b><a href="'+Page+'message.php?to='+Js[i]['c']+'"><img src="'+mug+'"/>'+JsA[Js[i]['c']]['n']+'</b></a> '+ ( (Js[i]['t'] != 0) ? '<span data-livestamp="'+Js[i]['t']+'"></span>': '' )+'</div><div class="MsgBxD-B">'+Msg+'</div></div>';
			}
		}
				...
				for(var i=0;i<Js.length;i++){
					...
					var stringToCheck=JsA[Js[i]['c']]['n'];
						if (stringToCheck.substr(0,query.length).toUpperCase() == query.toUpperCase()){
							...
							PstDa+='<div class="MsgBxD" style="display:none;" id="MsgBxD-'+Js[i]['c']+'" data-key="'+Js[i]['c']+'" data-value="'+Js[i]['t']+'"><div class="MsgBxD-T"><b><a href="'+Page+'message.php?to='+Js[i]['c']+'"><img src="'+mug+'"/>'+JsA[Js[i]['c']]['n']+'</b></a> '+ ( (Js[i]['t'] != 0) ? '<span data-livestamp="'+Js[i]['t']+'"></span>': '' )+'</div><div class="MsgBxD-B">'+Msg+'</div></div>';
						}
				}
					$("#PstDa").html(PstDa);				
			}
				...
				for(var i=0;i<Js.length;i++){
					...
					var Msg='Start Chat With Hiii';
						if(Js[i]['m'] != ''){
							...
							PstDa+='<div style="display:none;" class="MsgBxD" id="MsgBxD-'+Js[i]['c']+'" data-key="'+Js[i]['c']+'" data-value="'+Js[i]['t']+'"><div class="MsgBxD-T"><b><a href="'+Page+'message.php?to='+Js[i]['c']+'"><img src=" src="'+mug+'"/>'+JsA[Js[i]['c']]['n']+'</b></a> '+ ( (Js[i]['t'] != 0) ? '<span data-livestamp="'+Js[i]['t']+'"></span>': '' )+'</div><div class="MsgBxD-B">'+Msg+'</div></div>';
						}
				}
